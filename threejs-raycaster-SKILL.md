...
本技能以 **Experience Singleton 架構**為前提。Raycaster 邏輯集中在 `src/Experience/Raycaster.js`，透過 `new Experience()` 取得 singleton 後存取所需資源。
...
export default class Raycaster {
  ...
  _setMouseListeners() {
    ...
    this._onClick = () => {
      if (this.currentIntersect) {
        this.currentIntersect.object.dispatchEvent({ type: 'raycast-click' })
      }
    }
  ...
  update() {
    ...
    if (intersects.length) {
      if (!this.currentIntersect) {
        // 剛進入（相當於 mouseenter）
        intersects[0].object.dispatchEvent({ type: 'raycast-enter' })
      }
      this.currentIntersect = intersects[0]
    } else {
      if (this.currentIntersect) {
        // 剛離開（相當於 mouseleave）
        this.currentIntersect.object.dispatchEvent({ type: 'raycast-leave' })
      }
      this.currentIntersect = null
    }
  }
...
> **為什麼用 `dispatchEvent`？** 讓各個 World 子物件自行監聽自己的事件，不需要在 Raycaster 裡寫死邏輯，職責分離更清晰。
...
export default class InteractiveMeshes {
  ...
  _setRaycaster() {
    // 告知 Raycaster 要測試哪些物件
    this.raycaster.setObjectsToTest(this.meshes)

    for (const mesh of this.meshes) {
      mesh.addEventListener('raycast-enter', () => {
        mesh.material.color.set('#0000ff')
      })
      mesh.addEventListener('raycast-leave', () => {
        mesh.material.color.set('#ff0000')
      })
      mesh.addEventListener('raycast-click', () => {
...
_setRaycaster() {
  ...
  this.meshes[0].addEventListener('raycast-click', () => {
    console.log('clicked object 0 — 開門')
    this._openDoor()
  })
  this.meshes[1].addEventListener('raycast-click', () => {
    console.log('clicked object 1 — 播放動畫')
    this._playAnimation()
  })
  this.meshes[2].addEventListener('raycast-click', () => {
    console.log('clicked object 2 — 切換燈光')
    this._toggleLight()
  })
}
```
...
GLTF 模型是巢狀結構，直接傳入 `model`（`gltf.scene`），Raycaster 會遞迴偵測其所有子物件：
...
export default class Duck {
  ...
  _setModel() {
    ...
    // 模型載入後才設定 raycaster 測試目標
    this.raycaster.setObjectsToTest([this.model])

    // GLTF 子物件用 traverse 批次監聽
    this.model.traverse((child) => {
      if (child.isMesh) {
        child.addEventListener('raycast-enter', () => {
          this.model.scale.set(1.2, 1.2, 1.2)
        })
        child.addEventListener('raycast-leave', () => {
...
若需要偵測兩個動畫物件之間是否碰撞（例如雷射掃過場景），直接設定光線起點與方向，不需要滑鼠座標：
...
update() {
  ...
  this.instance.set(rayOrigin, rayDirection)
